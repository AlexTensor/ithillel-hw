...
function checkBalance(amount: number, list: User[]) {
    return list.filter((el: User): boolean => convertAmountToNumber(el.balance) < amount);
}

console.log(checkBalance(2000, users));

function totalAmount(list: User[]):number {
    const total: number = list.reduce((acc: number, el: User): number => acc + convertAmountToNumber(el.balance), 0);
    return Number(total.toFixed(2));
}

console.log(totalAmount(users));

function getRichUser(list: User[]):User {
...
type AccType = Record<string, User[]>;

function getNameSakes(list: User[]): User[] {
    const groupByName = list.reduce((acc: AccType, el: User): AccType => {
        if(!acc[el.name]){
            acc[el.name] = [];
        }
        acc[el.name].push(el);
    ...
    let nameSakesList: User[] = [];
    Object.keys(groupByName).forEach((key: string): void => {
...
function convertAmountToNumber(amount: string): number {
